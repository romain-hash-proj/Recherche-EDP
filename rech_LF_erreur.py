...
def calcul_erreur_L1(x, rho_num, Zl, Zr, t):
    ...
    # SÉCURITÉ : forcer 1D  CHATGPT
    rho_num   = np.squeeze(rho_num)
    rho_exact = np.squeeze(rho_exact)
    
    erreur = np.sum(np.abs(rho_num - rho_exact))
    norm=np.sum(np.abs(rho_exact))
    return float(erreur/norm)



#Nx = 500
liste_err=[]
liste_Nx = [50, 100, 500, 1000, 2000]
#liste_Nx=[100, 500, 1000, 5000]
for elt in liste_Nx:
    x = np.linspace(-5, 5, elt)
    dx = x[1] - x[0]
...
log_Nx=np.log(liste_Nx)
log_err=np.log(liste_err)
#plot de l'erreur
plt.figure()
plt.plot(log_Nx, log_err) #R-H
plt.xlabel("log Nx")
plt.ylabel("Erreur log")
plt.title("Erreur en fonction de Nx (lax friedrich)")
plt.show()

pente, C = np.polyfit(np.log(liste_Nx), np.log(liste_err), 1)
print(pente)